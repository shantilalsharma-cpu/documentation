...
.. seealso::
   `Odoo Tutorials: Lots & Serial Numbers
   <https://www.odoo.com/slides/slide/lots-serial-numbers-5325?fullscreen=1>`_
...
The *Lots & Serial Numbers* traceability feature must be enabled first to track products. To do so,
go to :menuselection:`Inventory app --> Configuration --> Settings`, scroll down to the
:guilabel:`Traceability` section, and select the checkbox next to :guilabel:`Lots & Serial Numbers`.
Be sure to click the :guilabel:`Save` button to save changes.
...
.. example::
   Enabling the *Create New* option for receipts allows new serial numbers to be assigned as items
   are received. However, for delivery orders, it is often disabled to prevent workers from
...
From the *Operations Types* page, select the desired operation type (e.g. :guilabel:`Receipts`,
:guilabel:`Delivery Orders`, or :guilabel:`Manufacturing`), and select the :guilabel:`Create New`
option in the :guilabel:`Lots/Serial Numbers` section of the operation type's configuration page.
...
After the :ref:`Lots & Serial Numbers setting <inventory/product_management/traceability-setting>`
is activated, individual products can now be tracked using serial numbers. To track a product, go to
:menuselection:`Inventory app --> Products --> Products`, and open the desired product.

In the :guilabel:`Tracking` field of the :guilabel:`General Information` tab,  select :guilabel:`By
Unique Serial Number`, then click the :icon:`fa-cloud-upload` :guilabel:`(Save manually)` icon to
save the changes. Now, existing or new serial numbers can now be selected and assigned to newly
received or manufactured batches of this product.
...
- Via the :ref:`Detailed Operations window <inventory/product_management/stock-move>` on a receipt
- :doc:`During a manufacturing order
  <../../../manufacturing/basic_setup/manufacturing_work_orders>` for a product tracked by lots or
  serial numbers
...
do so, go to :menuselection:`Inventory app --> Products --> Lots/Serial Numbers`, and click
:guilabel:`New`. Doing so reveals a blank lot or serial number form. On this form, a new lot or
serial number can be generated automatically.

First, add a :guilabel:`Product` to the form. A serial number is generated automatically with the
next available serial number.

.. tip::
   While Odoo automatically generates a new lot or serial number to follow the most recent number,
   it can be edited and changed to any desired number, by clicking the line under the
   :guilabel:`Lot/Serial Number` field, and changing the generated number.

Optionally assign a unique :guilabel:`Reference` number for additional traceability. Additionally,
the lot number can be made visible to a specific :guilabel:`Company`. A detailed description of this
specific lot/serial number can also be added in the :guilabel:`Description` area.

.. tip::
   The :guilabel:`Reference` number field is a space for manufacturers to input an additional unique
   number to allow for easier tracking. For instance, stock keeping unit (SKU) values may be used
   here.

When all desired configurations are complete, click the :icon:`fa-cloud-upload` :guilabel:`(Save)`
icon to save all changes.
...
On that product's detail form, click the :icon:`fa-bars` :guilabel:`Lot/Serial Numbers` smart button
...
.. image:: serial_numbers/enter-in-field.png
   :alt: Specify a value for the Serial Number field on receipt.

.. tip::
   To make the :guilabel:`Serial Numbers` field visible on a receipt or delivery order, in the
   :guilabel:`Operations` tab, click the :icon:`oi-settings-adjust` :guilabel:`(Adjust Settings)`
   icon, and in the drop-down menu, select the :guilabel:`Serial Numbers` checkbox.
...
In the *Detailed Operations* pop-up window, manually enter serial numbers in the
...
Assign multiple serial numbers at once by clicking the :guilabel:`Generate Serials/Lots` link in the
*Detailed Operations* pop-up window.

Doing so opens a new pop-up window, *Generate Serial numbers*, which contains a few fields:
...
- :guilabel:`Keep current lines`: Select this to keep existing serial numbers that may have been
  previously added. To replace existing serial numbers in the list, leave the box unchecked.

After filling out these fields, click the :guilabel:`Validate` button.

The newly generated serials now appear in the *Detailed Operations* window. Click :guilabel:`Save`
to update the :guilabel:`Quantity` field on the delivery order or receipt.

Import Serials/Lots
*******************

Another option for assigning multiple serial numbers at once is to click the :guilabel:`Import
Serials/Lots` link in the *Detailed Operations* pop-up window.

.. important::
   If the import link is not visible, ensure the :guilabel:`Create New` box is checked in the
   :ref:`receipt's configuration page <inventory/product_management/operation-type-setting>`.

Doing so opens the *Import Serials* pop-up window. Enter each serial number on a separate line in
the :guilabel:`Serial numbers` text field.
...
.. tip::
   To expedite this process, copy and paste serial numbers from an existing spreadsheet and add them
   to the :guilabel:`Serial numbers` text field.

Finally, click :guilabel:`Import`.

.. image:: serial_numbers/stock-move-import-serials.png
   :alt: Show Import Serials pop-up.

.. example::
   For a receipt with a :guilabel:`Demand` of `5.00` products, one product has already been assigned
   a serial number in the *Detailed Operations* pop-up window.

   So, in the :guilabel:`Import Serials` pop-up window, four serial numbers, `SN-2` through `SN-5`
   are assigned to the remaining products by entering the following in the :guilabel:`Serial
   numbers` input field:
   ...
   The :guilabel:`Keep current lines` option is selected to add these serial numbers **in addition**
   to the serial number, `SN-1`, that has already been assigned.

   .. image:: serial_numbers/import-serial.png
      :alt: Serial numbers are added to the Import Serials form.
...
<inventory/product_management/incoming-outgoing-products>`. Then, click the :icon:`fa-bars`
:guilabel:`Moves` smart button at the top of the page.

In the :guilabel:`Lot/Serial Number Name` column, manually type (or select from the drop-down menu)
...
When finished, click the receipt or delivery order's breadcrumb, and the assigned serial numbers are
automatically saved.
...
To include serial numbers on delivery slips, navigate to :menuselection:`Inventory app -->
Configuration --> Settings`. Scroll down to the :guilabel:`Traceability` section,
...
numbers are listed on delivery slips for products tracked by serial numbers after the delivery order
is validated.

To view serial numbers on delivery orders and delivery slips, navigate to
:menuselection:`Inventory --> Operations --> Deliveries` and select an order containing a product
tracked using serial numbers.

To view the serial numbers of products included in the order, make sure the :guilabel:`Operations`
tab is open, then click the :icon:`oi-settings-adjust` :guilabel:`(adjust)` icon to the right of
the tab. Ensure that the :guilabel:`Serial Numbers` checkbox is selected, which causes a
:guilabel:`Serial Numbers` column to appear. The serial numbers for each product included in the
...
:icon:`fa-print` :menuselection:`Print --> Delivery Slip`. The delivery slip is downloaded. Open the
delivery slip using the device's browser or file manager. Serial numbers are listed next to their
...
.. tip::
   By default, this list is grouped by :guilabel:`Location`. To group by product, clear the
   :guilabel:`Search` field, then click :menuselection:`Group By --> Product`.
...
To group by lot and serial numbers, first remove any default filters from the search bar. Then,
click in the field and select :guilabel:`Custom Group`, which opens a drop-down menu. From this
drop-down menu, select :guilabel:`Lot/Serial Number`.

Doing so reveals all existing serial numbers and lots. Expanded each row to show all quantities of
product assigned to that lot or serial number. For unique serial numbers that are not reused, there
should be one product per serial number.
...
reporting templates that display the :guilabel:`Lot/Serial Number` field or the ability to filter by
