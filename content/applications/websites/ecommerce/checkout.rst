...
When clicking the :guilabel:`Add to cart` button, the customer remains on the product page by default.
If the selected product has linked :ref:`optional products <ecommerce/products/cross_upselling>`
or :ref:`configured variants <ecommerce/categories_variants/product-variants>`,
a :guilabel:`Configure` pop-up opens. The customer can then choose between :icon:`fa-shopping-cart` :guilabel:`Add to cart`
(to stay on the product page) and :guilabel:`Go to Checkout` :icon:`fa-chevron-right`
(to be redirected to the cart).
To proceed directly to the cart or checkout, customers can use the :ref:`Buy Now <ecommerce/checkout/buy-now>` button.
