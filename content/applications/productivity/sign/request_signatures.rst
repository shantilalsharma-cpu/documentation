...
.. note::
   ...
   in *visualization mode*. Click :icon:`fa-pencil` :guilabel:`Duplicate & Edit` to create a copy
   of the template, which can then be edited.
...
- the avatar of the template's :guilabel:`Owner`; by default, the owner is the user who created the
  template. This can be changed in the :ref:`configuration settings of the template
...
- enable :guilabel:`Hide certificate key pages` to hide the :ref:`reference of the certificate of
  completion <sign/security/hash>` displayed by default on each page of the document(s)
...
- See the status of individual signers, per signature request:
  ...
  - When a signature request is canceled, the signers name appears in red with a :icon:`fa-times`
